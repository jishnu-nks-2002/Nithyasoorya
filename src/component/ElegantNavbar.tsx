...
const DROPDOWN_DATA = {
  ...
  "SACRED SPACES": {
    featured: {
      title: "Our Story",
      subtitle: "Rooted in crystal energy & artisan craft",
      accent: "#9b4dca",
      tag: "Est. 2019",
    },
    sections: [
...
// ── Logo: image with graceful fallback to diamond SVG ────────────────────────
const Logo = () => {
  const [imgError, setImgError] = useState(false);

  if (imgError) {
    // Fallback SVG diamond if image fails to load
    return (
  ...
  return (
    <img
      ...
      style={{
        height: 40,                    // adjust to taste — 36–48px works well
        width: "auto",
        objectFit: "contain",
        display: "block",
        // keep colours consistent: remove background tint on light logos
...
const DropLink = ({ link, accentColor }: { link: { label: string; sub?: string; href: string }; accentColor: string }) => {
    ...
    >
      ...
      {link.sub && (
        <span style={{
          ...
          color: hov ? `${accentColor}99` : "#a090bb",
...
const DropPanel = ({ data, visible, alignRight }: {
  data: typeof DROPDOWN_DATA["COLLECTIONS"];
  visible: boolean;
  alignRight: boolean;
}) => {
    ...
    >
      ...
      <div style={{
        ...
        <div style={{
          ...
          <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 20, fontWeight: 500, lineHeight: 1.2, color: "#1e1030", margin: 0 }}>
            {data.featured.title}
          </p>
          <p style={{ fontFamily: "'Jost', sans-serif", fontSize: 10.5, fontWeight: 300, lineHeight: 1.55, color: "#7a6a99", margin: 0 }}>
          ...
          <a
            ...
            style={{
              ...
              letterSpacing: "0.18em", textTransform: "uppercase",
              color: data.featured.accent, textDecoration: "none",
          ...
          {data.sections.map((section, si) => (
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8, paddingLeft: 10 }}>
                ...
                <span style={{ fontFamily: "'Jost', sans-serif", fontSize: 8.5, fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", color: section.brandColor }}>
                  {section.brand}
                </span>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
                {section.links.map(link => (
                  <DropLink key={link.label} link={link} accentColor={section.brandColor} />
                ))}
              </div>
            </div>
          ))}
...
const NavItem = ({ item, navHidden, openMenu, setOpenMenu }: {
  ...
  const show = () => { clearTimeout(timeout.current!); if (!navHidden && item.hasDropdown) setOpenMenu(item.label); };
  const hide = () => { timeout.current = setTimeout(() => setOpenMenu(p => p === item.label ? null : p), 130); };

  useEffect(() => { if (navHidden) setOpenMenu(null); }, [navHidden]);
  ...
  return (
    <div style={{ position: "relative" }} onMouseEnter={show} onMouseLeave={hide}>
      ...
      {item.hasDropdown && dropData && (
        <DropPanel data={dropData} visible={isOpen} alignRight={alignRight} />
      )}
    </div>
  );
};

export default function EmpavaiNavbar() {
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .empavai-navlink {
          ...
          letter-spacing: 0.14em; color: #4a3966;
        ...
        .empavai-shopbtn {
          ...
          letter-spacing: 0.14em; color: #fff !important;
          background: #7c5cbf; border: none; border-radius: 50px;
        ...
        .empavai-shopbtn:hover {
          background: #6344a8; transform: translateY(-1px);
        ...
        .en-mob-trigger {
          ...
          letter-spacing: 0.14em; text-transform: uppercase; color: #4a3966;
          text-decoration: none; text-align: left; transition: color 0.2s ease;
        }
        ...
        .en-mob-link {
          ...
          margin-bottom: 2px; transition: background 0.18s ease;
        }
        .en-mob-link:hover { background: rgba(124,92,191,0.06); }
        .en-mob-link-label { font-family:'Jost',sans-serif; font-size:12.5px; font-weight:400; color:#2e1f4a; }
        .en-mob-link-sub   { font-family:'Jost',sans-serif; font-size:10px;   font-weight:300; color:#a090bb; }
        ...
        .en-mob-viewall {
          ...
          letter-spacing: 0.2em; text-transform: uppercase;
          color: #7c5cbf; text-decoration: none;
      ...
      <nav
        ...
        style={{
          ...
          background: isScrolled ? "rgba(248,246,254,0.96)" : "rgba(248,246,254,0.98)",
      ...
      >
        ...
        >
          {/* Logo image — swap src to your actual path */}
          <Logo />

          {/* Brand wordmark — hide if your logo already contains text */}
          <div style={{ lineHeight: 1 }}>
            <div style={{
              fontFamily: "'Cormorant Garamond', serif",
              fontSize: 17, fontWeight: 600,
              letterSpacing: "0.22em", color: "#2e1f4a",
              textTransform: "uppercase",
            }}>
              EMPAVAI
            </div>
            <div
              className="empavai-sub"
              style={{
                fontFamily: "'Jost', sans-serif",
                fontSize: 8.5, fontWeight: 400,
                letterSpacing: "0.22em", color: "#9b82c4",
        ...
        <a href="/collections" className="empavai-shopbtn empavai-deskshop">SHOP NOW</a>
      ...
      {/* ── Mobile drawer ── */}
      <div className={`en-mob-drawer ${mobileOpen ? "open" : ""}`} aria-hidden={!mobileOpen}>
        {NAV_ITEMS.map(item => {
          const dropData = DROPDOWN_DATA[item.label as keyof typeof DROPDOWN_DATA];
          ...
          if (!dropData) {
            return (
              <div key={item.label} className="en-mob-row">
                <a href={item.href} className="en-mob-trigger" onClick={() => setMobileOpen(false)}>
                  {item.label}
                </a>
              </div>
            );
          ...
          return (
            <div key={item.label} className="en-mob-row">
              <button
                className="en-mob-trigger"
                onClick={() => setMobileExp(expanded ? null : item.label)}
